...
if __name__ == '__main__':
    ...
    if args.OP:
        if args.OP=='BIT':
            print('BIT selected')
            live_capture = LiveCapture(align=0,is_framos=True)
            if args.VF:
            ...
            # order_max_dif = float(config['processing']['order_max_dif'])
            # # boxes_to_shift = np.asarray(
            # #     [int(x) for x in config['processing']['boxes_shift'].split(',') if x.strip().isdigit()])
            #
            # liftCoeffs = np.array([float(config['lift']['coefX']),float(config['lift']['coefY']),float(config['lift']['coefZ'])])
            #
            # # [capture]
            ...
            if args.FF:
                ...
                capt = LiveCapture(align=settings.capt_align_to,is_framos=settings.isFramos,ROI = settings.ROI,A_factor=settings.A_factor,max_laser=settings.max_laser)
                try:
                    capt.start()
                except ConnectionError as e:
                    print(e)
            ...
            print("order selected: {}".format(order)) # Ziv 055020 added print of order for debug purposes (will be saved to STA log)
            ...
            print("time from load: {}".format(end - data_load_end))
            print("time from find media: {}".format(end - find_media_end))
            print("time from rotation: {}".format(end - rotation_end))
            print("time from surface processing: {}".format(end - surface_processing_end))
            ...
            print (args.OP + ' is not a supported operation')
