...
class Application:
    def __init__(self, master, config_file='config.ini'):
        ...
        if self.capt_use_live_capture:
            self.capt = LiveCapture(align=self.capt_align_to,is_framos=self.isFramos,ROI = self.ROI,A_factor=self.A_factor,max_laser=self.max_laser)
            try:
                self.capt.start()
    ...
    def calcParams(self):
        ...
        # lowpass = ndimage.gaussian_filter(self.z , 7)
        # gauss_highpass = self.z - lowpass
        # gauss_highpass[np.abs(gauss_highpass)>0.1]=0
        # lowpass = self.z-gauss_highpass

        # import scipy.fftpack as fp
        # F1 = fp.fft2((self.z).astype(float))
        # F2 = fp.fftshift(F1)
        # (w , h) = self.z.shape
        ...
        # F3=np.zeros_like(F2)
        # F3[half_w - n:half_w + n + 1 , half_h - n:half_h + n + 1] = F2[half_w - n:half_w + n + 1 , half_h - n:half_h + n + 1]
        # lowpass = fp.ifft2(fp.ifftshift(F3)).real
        ...
        # lp_point_array = np.vstack(
        #     (self.x[cimg == 255].flatten() , self.y[cimg == 255].flatten() , lowpass[cimg == 255].flatten())).transpose()
        # lp_point_array = lp_point_array[lp_point_array[: , 2] > 0 , :]
        ...
        if self.cal_perform_rotation:  # for debug only!!!!!!  use False for real apps
            ...
            # lp_rotatedPoints = self.cal.rotate_about_calibration(lp_point_array ,
            #                                                   coefficients=None ,
            #                                                   with_print_log=self.cal_display_angles ,
            #                                                   dual_rotation=self.cal_perform_dual_rotation)
        else:
        ...
        if False: # for debug only!!!!!!  use False for real apps
            ...
            # lp_csdata , lp_C , lp_res = sfnz.fit_clean_surf(lp_rotatedPoints , order=3 , iter=5 , coeff_mx_change=0.01)
            ...
            sbs = SmoothBivariateSpline(csdata[: , 0] , csdata[: , 1] , csdata[: , 2])
            zsbs = sbs.ev(csdata[: , 0] , csdata[: , 1] )

            cartcoord = list(zip(csdata[: , 0] , csdata[: , 1]))
            interp = LinearNDInterpolator(cartcoord , zsbs , fill_value=0)

            X , Y = np.meshgrid(
                np.arange(np.min(csdata[: , 0]) , np.max(csdata[: , 0]) , 0.002) ,
                np.arange(np.min(csdata[: , 1]) , np.max(csdata[: , 1]) , 0.002))
            ...
            interp = LinearNDInterpolator(cartcoord , csdata[:, 2] , fill_value=0)

            Z0 = interp(X , Y)

            # lp_cartcoord = list(zip(lp_rotatedPoints[: , 0] , lp_rotatedPoints[: , 1]))
            # lp_interp = LinearNDInterpolator(lp_cartcoord , lp_rotatedPoints[: , 2] , fill_value=0)
            #
            # lp_Z0 = lp_interp(X , Y)
            ...
            outfile = self.capt_saving_subfolder + '\\' + now.strftime("interpolated_filtered_%d%m%Y_%H%M%S.csv")
            np.savetxt(outfile , Z0_sbs , delimiter=',')
