...
class LiveCapture:
    ...
    def __init__(self, align=0, path="", filename="",is_framos=False,ROI = None, A_factor=0.08, max_laser = False, exposure = 500.000 ):
        """
        ...
        self.color_filename = ""
        self.ir_filename = ""
        self.is_framos = is_framos
        self.ROI = ROI
        self.resX = 640
        self.resY = 480
        self.frameRate = 30
        self.A_factor = A_factor
        self.max_laser = max_laser
    ...
    def start(self):
        ...
        if self.is_framos:
            ...
            # self.cfg.enable_stream(rs.stream.infrared , 1 , self.resX , self.resY , rs.format.y8 , self.frameRate)
            # self.cfg.enable_stream(rs.stream.infrared , 2 , self.resX , self.resY , rs.format.y8 , self.frameRate)
            # self.cfg.enable_stream(rs.stream.depth , self.resX , self.resY , rs.format.z16 )
            # self.cfg.enable_stream(rs.stream.color , self.resX , self.resY , rs.format.bgr8)
            # self.cfg.enable_stream(rs.stream.infrared , 1 , self.resX , self.resY , rs.format.y8 )
            # self.cfg.enable_stream(rs.stream.infrared , 2 , self.resX , self.resY , rs.format.y8 )
        ...
        if self.cfg.can_resolve(self.pipe):
            ...
            # s = self.device_profile.get_device().first_roi_sensor()
            # roi = s.get_region_of_interest()

            s = self.device_profile.get_device().query_sensors()[1]
            s.set_option(rs.option.exposure , self.exposure)


            device = self.device_profile.get_device()  # type: rs.device
            depthSensor = device.first_depth_sensor()  # type: rs.depth_sensor

            if depthSensor.supports(rs.option.depth_units):
                depthSensor.set_option(rs.option.depth_units , 0.0001)

            advnc_mode = rs.rs400_advanced_mode(device)
            factor = rs.STAFactor()
            factor.a_factor = self.A_factor
            ...
            #     roi.max_y = 170
            # else:
            #     roi.min_x = self.ROI[0]
            #     roi.min_y = self.ROI[2]
            #     roi.max_x = self.ROI[1]
            #     roi.max_y = self.ROI[3]


            # s.set_region_of_interest(roi)

            print("Pipeline has started")
            if self.max_laser: # TODO - This is a placeholder for setting te laser power - need tests before integration
    ...
    def capture_average_reading(self, dry_runs=5, images_2_mean_count=10, mean_flag=False, single_capture_attempts=5, single_capture_timeout_ms=1000):
        ...
        if dry_runs > 0:
            ...
            for i in range(dry_runs):
                try:
                    ...
                    continue
            capture_end = time()
            print("dry runs capture time: {}".format(capture_end - capture_start))
            [self.res_y, self.res_x] = tmp_f_d.shape


        depth_array = np.zeros((self.res_y, self.res_x, images_2_mean_count),dtype=np.uint16)
        ir_array = np.zeros((self.res_y, self.res_x, images_2_mean_count),dtype=np.uint8)
        color_array = np.zeros((self.res_y, self.res_x, 3, images_2_mean_count),dtype=np.uint8)
        ...
        # capture_end = time()
        # print("capture loop time: {}".format(capture_end - capture_start))
        ...
        if mean_flag:
            ...
            average_ir_image = np.mean(ir_array, axis=2) # Ziv 190520 - leaving median for depth only
            average_color_image = np.mean(color_array, axis=3) # Ziv 190520 - leaving median for depth only
        # capture_end = time()
        # print("image stack averaging time: {}".format(capture_end - capture_start))

        # capture_start = time()
        x, y, z = lh.convert_depth_frame_to_pointcloud(average_depth_image, depth_intrinsics, depth_scale)
        # capture_end = time()
        # print("point cloud conversion time: {}".format(capture_end - capture_start))
